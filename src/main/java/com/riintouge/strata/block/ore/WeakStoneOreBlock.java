...
public class WeakStoneOreBlock extends OreBlockBase implements IMetaPropertyProvider
{
    public static final WeakStoneOreBlock INSTANCE = new WeakStoneOreBlock();
    public static final String REGISTRY_NAME = "strata:weak_stone_ore";
    public static final String UNLOCALIZED_NAME = REGISTRY_NAME;

    static
    {
        DynamicOreHostManager.INSTANCE.registerOreBlock( REGISTRY_NAME , PropertyWeakStoneOre.PROPERTY );
    }

    public WeakStoneOreBlock()
    {
        super( Material.ROCK );

        setDefaultState( blockState.getBaseState().withProperty( PropertyWeakStoneOre.PROPERTY , PropertyWeakStoneOre.DEFAULT ) );
        setHardness( 3f ); // TODO: Utilize hardness for player.addExhaustion() ?
    ...
    public void getDrops( NonNullList<ItemStack> drops , IBlockAccess world , BlockPos pos , IBlockState state , int fortune )
    {
        ...
        PropertyWeakStoneOre.Type ore = StateUtil.getValue( extendedState , PropertyWeakStoneOre.PROPERTY , PropertyWeakStoneOre.DEFAULT );
        int fortuneBonus = fortune > 0 ? RANDOM.nextInt( fortune + 1 ) : 0;
        drops.add( new ItemStack( Items.weakStoneOreItem , 1 + fortuneBonus , ore.getValue() ) );
    }

    @Override
    public IBlockState getStateFromMeta( int meta )
    {
        return getDefaultState().withProperty( PropertyWeakStoneOre.PROPERTY , PropertyWeakStoneOre.types()[ meta ] );
    }
